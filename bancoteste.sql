...
--
-- Host: 127.0.0.1
-- Tempo de geração: 23-Mar-2024 às 01:58
...
--
-- Estrutura stand-in para vista `vendascommeses`
...
--
-- Estrutura para vista `vendascommeses`
--
DROP TABLE IF EXISTS `vendascommeses`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vendascommeses`  AS SELECT `vendas`.`venda_id` AS `venda_id`, `vendas`.`cliente_id` AS `cliente_id`, `vendas`.`produto_id` AS `produto_id`, date_format(`vendas`.`data_venda`,'%M %Y') AS `mes_descritivo`, `vendas`.`valor_venda` AS `valor_venda` FROM `vendas` ;
